...
2、mqtts mutual ca sender task接收"CONNECT OK"、"PUBLISH_REQ"、"PUBLISH OK"三种类型的"MQTT_EVENT"消息，遍历队列send_queue，逐条发送数据到server；
3、mqtts mutual ca sender task接收"DISCONNECTED"类型的"MQTT_EVENT"消息，丢弃掉队列send_queue中未发送的数据；
...
-- mqtts mutual ca client的任务名前缀
mqtts_m_ca_sender.TASK_NAME_PREFIX = "mqtts_m_ca_"

-- mqtts_mutual_ca_client_sender的任务名
mqtts_m_ca_sender.TASK_NAME = mqtts_m_ca_sender.TASK_NAME_PREFIX.."sender"
...
-- mqtts_m_ca client sender的任务处理函数
local function mqtts_m_ca_client_sender_task_func() 
