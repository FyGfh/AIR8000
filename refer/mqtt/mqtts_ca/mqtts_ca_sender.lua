...
local function send_data_req_proc_func(tag, topic, payload, qos, cb)
    ...
    sys.sendMsg(mqtts_ca_sender.TASK_NAME, "MQTT_EVENT", "PUBLISH_REQ")
end
...
local function mqtts_ca_client_sender_task_func() 
    ...
    while true do
        -- 等待"MQTT_EVENT"消息
        msg = sys.waitMsg(mqtts_ca_sender.TASK_NAME, "MQTT_EVENT")
...
sys.taskInitEx(mqtts_ca_client_sender_task_func, mqtts_ca_sender.TASK_NAME)
